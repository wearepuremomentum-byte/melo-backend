...
# ============================================
# MODELS — kept minimal, no new columns
# ============================================
...
class User(db.Model):
    ...
    subscription_status = db.Column(db.String, default='trial')  # trial, active, canceled
...
class Subscription(db.Model):
    ...
    tier = db.Column(db.String(20), default='base')  # base, pro, premium
...
TIER_PRICES = {
    'base':    {'amount': 2000,  'name': 'Pure Meditation – Base ($20/mo)'},
    'pro':     {'amount': 2999,  'name': 'Pure Meditation – Pro ($29.99/mo)'},
    'premium': {'amount': 4999,  'name': 'Pure Meditation – Premium ($49.99/mo)'},
}
...
def get_user_tier(user):
    ...
    return sub.tier if sub else 'base'
...
def get_tracks():
    ...
    return jsonify([{
        ...
        'duration': t.duration_minutes, 'description': t.description, 'url': t.audio_url
    } for t in tracks]), 200
...
def play_track():
    ...
    # Only PAID active subscribers get unlimited
    if user.subscription_status == 'active':
    ...
    # Trial and free users use 30-min monthly credits
    if user.credits_remaining < track.duration_minutes:
        return jsonify({'error': 'Not enough credits. Upgrade to continue.'}), 402
...
def upgrade():
    """Create Stripe checkout for base, pro, or premium"""
    data = request.json
    user_id = data.get('user_id')
    tier = data.get('tier', 'base')

    if tier not in TIER_PRICES:
        return jsonify({'error': 'Invalid tier. Must be base, pro, or premium'}), 400
...
def checkout():
    """Legacy checkout — defaults to base tier"""
    data = request.json
    data['tier'] = 'base'
...
def webhook():
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        tier = session['metadata'].get('tier', 'base')
        ...
        if user_id:
            ...
            if user:
                ...
                user.credits_remaining = 9999  # unlimited for paid
    ...
    if event['type'] == 'customer.subscription.deleted':
        ...
        if sub:
            ...
            if user:
                ...
                user.credits_remaining = 30  # back to free tier
            db.session.commit()
...
def cancel():
    ...
    db.session.commit()
    return jsonify({'status': 'canceled', 'message': 'Subscription canceled.'}), 200
