...
# ============================================
# INIT DEMO TRACKS (OPTIONAL)
# ============================================

@app.route('/api/init-demo-tracks', methods=['POST'])
def init_demo_tracks():
    """Initialize database with demo meditation tracks"""
    try:
        # Check if tracks already exist
        existing_tracks = Track.query.count()
        if existing_tracks > 0:
            return jsonify({'status': 'already_initialized', 'tracks': existing_tracks}), 200

        # Create demo tracks
        demo_tracks = [
            # Sleep
            Track(title='10 Min Deep Sleep', category='sleep', duration_minutes=10,
                  audio_url='https://example.com/sleep1.mp3', description='Quick sleep meditation'),
            Track(title='30 Min Relaxation', category='sleep', duration_minutes=30,
                  audio_url='https://example.com/sleep2.mp3', description='Deep relaxation'),
            Track(title='60 Min Sleep Story', category='sleep', duration_minutes=60,
                  audio_url='https://example.com/sleep3.mp3', description='Long sleep meditation'),
            
            # Focus
            Track(title='20 Min Focus Boost', category='focus', duration_minutes=20,
                  audio_url='https://example.com/focus1.mp3', description='Improve concentration'),
            Track(title='30 Min Deep Work', category='focus', duration_minutes=30,
                  audio_url='https://example.com/focus2.mp3', description='Deep work flow'),
            Track(title='45 Min Flow State', category='focus', duration_minutes=45,
                  audio_url='https://example.com/focus3.mp3', description='Enter flow state'),
            
            # Anxiety
            Track(title='10 Min Calm', category='anxiety', duration_minutes=10,
                  audio_url='https://example.com/anxiety1.mp3', description='Quick anxiety relief'),
            Track(title='15 Min Grounding', category='anxiety', duration_minutes=15,
                  audio_url='https://example.com/anxiety2.mp3', description='Grounding meditation'),
            Track(title='20 Min Peace', category='anxiety', duration_minutes=20,
                  audio_url='https://example.com/anxiety3.mp3', description='Inner peace'),
            
            # Energy
            Track(title='5 Min Morning Boost', category='energy', duration_minutes=5,
                  audio_url='https://example.com/energy1.mp3', description='Morning energy'),
            Track(title='10 Min Energy Flow', category='energy', duration_minutes=10,
                  audio_url='https://example.com/energy2.mp3', description='Full body energy'),
        ]
        
        db.session.add_all(demo_tracks)
        db.session.commit()
        
        return jsonify({
            'status': 'initialized',
            'tracks_added': len(demo_tracks),
            'message': 'Demo tracks successfully added'
        }), 200
